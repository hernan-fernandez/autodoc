...
The server <b> $(hostname) </b> is $(uname) server and has an uptime of $uptime_dias since last reboot.
...
$ip</pre>"
        ...
        count_ip=$(echo "$ip" | grep -v '^$' -c )

        if [ $(( count_ip )) -ne 1  ]; then
...
dmidecode -t system |grep -i  "Serial Number"
        ...
        echo "        Number of CPU : "; grep -c "processor" /proc/cpuinfo
        echo "        CPU Model     : "; grep "model name" /proc/cpuinfo | sort -u |awk -F":" '{print $2}'
        echo "        Meminfo       : "; grep "MemTotal" /proc/meminfo | awk '{print $2 $3}'
        echo "</pre>
        <div class='page-break'></div>"
}
...
DetectLinuxRole () {
        ...
        psoutput=$(ps -o comm  --ppid 2 -N |sort -u |grep -Evi COMMAND\|grep\|ps\|sort\|hald\|nrpe\|ntpd\|dbus\|pickup\|qmgr\|chronyd\|bash\|avahi\|dnsmasq\|lsmd\|sshd\|sftp-server\|rpc\|polkitd\|uuidd\|sh\|tail\|cat\|postdrop\|sftp\|sendmail\|cleanup\|bounce\|nagios\|crond\|gnome\|gconf\|metacity\|snmp\|gvfsd\|bonobo\|gdm\|portmap\|pulseaudio\|rtkit-daemon\|at-spi-registry\|perl\|python\|xfs\|postmaster)
        ...
        if [ $exit_status -eq 1 ]; then
                ...
                psoutput=$(ps -fea | awk '{ print $8}' |sort -u |grep -Evi COMMAND\|grep\|ps\|sort\|hald\|nrpe\|ntpd\|dbus\|pickup\|qmgr\|chronyd\|bash\|avahi\|dnsmasq\|lsmd\|sshd\|sftp-server\|rpc\|polkitd\|uuidd\|sh\|tail\|cat\|postdrop\|sftp\|sendmail\|cleanup\|bounce\|nagios\|crond\|gnome\|gconf\|metacity\|snmp\|gvfsd\|bonobo\|gdm\|portmap\|pulseaudio\|rtkit-daemon\|at-spi-registry\|perl\|python\|xfs\|postmaster)
        fi


        cant_process=$(echo "$psoutput" | grep -cv ^$)

        echo "<pre>"

        if [ $(( cant_process )) -gt 0 ]; then
                apache=$(echo "$psoutput" |grep -Eci apache\|httpd)
                nginx=$(echo "$psoutput" |grep -ci nginx)
                oracle=$(echo "$psoutput" |grep -Eci oracle\|ora_)
                mysql=$(echo "$psoutput" |grep -Eci mysqld\|mariadb)
                postgresql=$(echo "$psoutput" |grep -ci postgres)
                sap=$(echo "$psoutput" |grep -Eci sap\|jstart)
                java=$(echo "$psoutput" |grep -Eci java)
                weblogic=$(echo "$psoutput" |grep -Eci WebLogic\|startNodeManage)
                dns=$(echo "$psoutput" |grep -Eci named\|bind)
...
ServicesAutoStart () {
        ...
        systemctl list-unit-files |grep enabled
                ...
                echo "Runlevel 2"
                find /etc/rc2.d/ -type l | awk '{print $NF}' |sort -u
                echo "Runlevel 3"
                find /etc/rc3.d/ -type l | awk '{print $NF}' |sort -u

                find /sbin/rc3.d/ -type l | awk '{print $NF}' |sort -u

                echo "Runlevel 5"
                find /etc/rc5.d/ -type l | awk '{print $NF}' |sort -u
        fi
        echo "</pre>
        <div class='page-break'> </div>"
}
...
DiskConfigLinux () {
        ...
        count_mount=$(grep -cv '^$' "$remote_mount")

        if [ $(( count_mount )) -gt 0 ]; then
